...
while camera.isOpened():
	...
	for class_id, score, bounding_box in zip(class_ids, scores,
		...
		if class_name in active_buttons:
			cv2.putText(frame, str(class_name), (x, y - 10), cv2.FONT_HERSHEY_PLAIN, 1, color, 2)
			cv2.putText(frame, str(score), (x+100, y - 10), cv2.FONT_HERSHEY_PLAIN, 1, color, 2)
			cv2.rectangle(frame, (x, y), (x + w, y + w), color, 3)
